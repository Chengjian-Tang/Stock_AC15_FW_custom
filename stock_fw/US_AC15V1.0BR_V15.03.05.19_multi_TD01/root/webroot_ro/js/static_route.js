    ...
    G.validate = $.validate({
        custom: function () {
            ...
            $("#portBody tr").each(function () {
                ...
                existMask = existMask > mask ? mask : existMask;

                if (checkIpInSameSegment(network, existMask, existIP, existMask)) {
                    netExist = true;
                    return false;
                }
            });
...
/**
 * 掩码与IP求与,匹配是否为同网段
 * @param  {[type]} eip   [a数据目标网络ip]
 * @param  {[type]} emask [a数据子网掩码]
 * @param  {[type]} ip    [b数据目标网络ip]
 * @param  {[type]} mask  [b数据子网掩码]
